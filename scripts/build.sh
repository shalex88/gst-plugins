...
#!/bin/bash -e
cd /workspace/submodules/orin/gst-plugins
...
EOFSCRIPT
        ...
        DOCKER_ARGS="-v $PROJECT_ROOT:/workspace -v $TEMP_SCRIPT:/tmp/docker_build.sh -w /workspace/submodules/orin/gst-plugins"
        if [ -n "${INSTALL_ROOT:-}" ]; then
...
cd $ROOT_DIR/plugins/gstmyf2f || exit 1
mkdir -p "$BUILD_DIR"

{
    ...
    echo "Build log saved to plugins/gstmyf2f/$LOG_FILE"
    echo "Build completed at $(date)"
    exit $BUILD_EXIT
} 2>&1 | tee "$LOG_FILE"

cd $ROOT_DIR/processing-libs/f2f/nv-stabilizer || exit 1
mkdir -p "$BUILD_DIR"

{
    ...
    echo "Build log saved to processing-libs/f2f/nv-stabilizer/$LOG_FILE"
    echo "Build completed at $(date)"
    exit $BUILD_EXIT
} 2>&1 | tee "$LOG_FILE"
